...
func SelectFormulacao(con *gorm.DB, filtroFormulacao models.Formulacao) (formulacoes []models.Formulacao) {
	...
	// rows := con.Where("codigo LIKE ? OR obs LIKE ? ", "%"+filtroFormulacao.Codigo+"%", "%"+filtroFormulacao.Obs+"%").Find(&formulacoes)
	// defer rows.Close()
	// if err != nil {kkkkkk
	// 	panic(err.Error())
	// }
	...
	// for rows.Next() {
	// 	err = rows.Scan(&f.Id, &f.QtdEstimadaProduzida, &f.Icm, &f.Pis, &f.Cofins, &f.Comissao,
...
func SelectFormulacaoId(con *gorm.DB, id int64) (formulacao models.Formulacao) {
	...
	// 		&formulacao.Cofins, &formulacao.Comissao, &formulacao.MargemLucro, &formulacao.DespesaFixa,
...
func InsertFormulacao(con *gorm.DB, formulacao *models.Formulacao) {
	...
	// _, err := stmt.Exec(&formulacao.QtdEstimadaProduzida, &formulacao.Icm, &formulacao.Pis, &formulacao.Cofins,
...
func UpdateFormulacao(con *gorm.DB, formulacao *models.Formulacao) {
...
func DeleteFormulacao(con *gorm.DB, id int64) {
